...
Usage:
    python train_autoencoder.py --config config.json
    python train_autoencoder.py --config config.json --out_path results/experiment1
"""
...
from autoencoder import ConvAutoencoder, AutoencoderTrainer
...
def main():
    ...
    for epoch in range(config['epochs']):
        ...
        # Log metrics
        logger.info(f"  train_loss: {train_metrics['train_loss']:.6f} | val_loss: {val_metrics['val_loss']:.6f}")
    ...
    # Save full autoencoder weights
    weights_path = os.path.join(args.out_path, 'autoencoder.pth')
    torch.save(model.state_dict(), weights_path)
    logger.info(f"Autoencoder weights saved to {weights_path}")
    
    # Save encoder weights separately (for later use as feature extractor)
    encoder_path = os.path.join(args.out_path, 'encoder.pth')
    torch.save(model.encoder.state_dict(), encoder_path)
    logger.info(f"Encoder weights saved to {encoder_path}")
    
    # Save encoder config for easy reconstruction
    encoder_config_path = os.path.join(args.out_path, 'encoder_config.json')
    with open(encoder_config_path, 'w') as f:
        json.dump(model.encoder.get_config(), f, indent=2)
    logger.info(f"Encoder config saved to {encoder_config_path}")
    ...
    logger.info("=" * 50)
    logger.info("To use the encoder as a feature extractor:")
    logger.info("  from autoencoder import Encoder")
    logger.info(f"  encoder = Encoder(**config)  # use encoder_config.json")
    logger.info(f"  encoder.load_state_dict(torch.load('{encoder_path}'))")
    logger.info("  features = encoder(images)")
